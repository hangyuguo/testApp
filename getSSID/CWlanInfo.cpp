...
CString CWlanInfo::getCurrentSSID()
{
	acquireWLANInformation();
	return m_csSSID;
}

bool CWlanInfo::acquireWLANInformation()
{
	bool bRet = true;
	m_csSSID.Empty();
			...
			if (pIfInfo->isState == wlan_interface_state_connected) {
				...
				if (dwResult != ERROR_SUCCESS) {
					...
					if (pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength == 0)
						...
						m_csSSID = (CString)pConnectInfo->wlanAssociationAttributes.dot11Ssid.ucSSID;
					}
